...
BEGIN
	...
	SELECT 
		...
		,	CASE
				WHEN po.FK_ICProductAttributeTTMTID IS NULL OR po.FK_ICProductAttributeTTMTID = 0 
					THEN ISNULL(ttmt.FK_ICProductAttributeTTMTID, 0)
				ELSE po.FK_ICProductAttributeTTMTID
			END AS FK_ICProductAttributeTTMTID
		,	so.ARSaleOrderType
		,	po.ARProductionPlanningType
		--,	ISNULL(ARSaleOrderItemHTID, 0) AS FK_ICProductAttributeHTTypeID
		,	ISNULL(ttmt.FK_ICProductAttributeTTMTID, 0) AS FK_ICProductAttributeHTTypeID
		,  ttmt.ARSaleOrderItemHTID
		,  ttmt.ARSaleOrderItemHTText
	FROM ARProductionPlanningItems ppi 
		...
		OUTER APPLY
		(
			SELECT TOP 1 soi.FK_ICProductAttributeTTMTID ,soi.ARSaleOrderItemHTID, soi.ARSaleOrderItemHTText
