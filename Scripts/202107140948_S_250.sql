...
BEGIN
	...
	DECLARE 
		...
		@ReceiptCostTotalFees		DECIMAL(18,5),
	...
	SELECT	@ReceiptNo						= i.ICReceiptNo
			...
			, @ReceiptCostTotalFees			= (SELECT ROUND(SUM(ISNULL(APFeePaymentReceiptAllocationCost,0.00)),2) 
												FROM  dbo.APFeePaymentReceipts
												WHERE AAStatus = 'Alive'
												AND FK_ICReceiptID = @ReceiptID)
	...
	(
		...
		ReceiptItemCostFees					DECIMAL(18,5),
		FK_APPurchaseOrderID				int,
	...
	insert into @ReceiptItemTable
		select ii.ICReceiptItemID
			...
			, ISNULL((ii.ICReceiptItemTotalCost / CASE WHEN @ReceiptSubTotalCost = 0 THEN 1 ELSE @ReceiptSubTotalCost END) * @ReceiptCostTotalFees, 0)
			...
			,(
							SELECT t.FK_ACAccountID FROM  dbo.APFeePaymentReceipts d
								INNER JOIN dbo.ACTransactions t ON t.ACTransactionReferenceID = d.FK_ACDocumentID AND t.ACTransactionModuleName = 'FeePayment'
							WHERE d.AAStatus = 'Alive'
							AND d.FK_ICReceiptID = ii.FK_ICReceiptID
							AND t.ACTransactionDebitAmount > 0
							AND t.ACTransactionDetailDesc = N'Chi phí mua hàng'
							)
		...
		BEGIN 
			...
			WHERE ReceiptItemID = (SELECT MAX(ReceiptItemID) FROM @ReceiptItemTable)
		END	
		--Update Chênh lệch chi phí mua hàng--
		SET  @ChenhLech = @ReceiptCostTotalFees - (SELECT SUM(ReceiptItemCostFees) FROM @ReceiptItemTable )		
		IF (ISNULL(@ChenhLech,0) <> 0)
		BEGIN 
			UPDATE r
			SET r.ReceiptItemCostFees = r.ReceiptItemCostFees + @ChenhLech
			FROM @ReceiptItemTable r
			WHERE ReceiptItemID = (SELECT MAX(ReceiptItemID) FROM @ReceiptItemTable)
		END	
	...
	begin	
		...
		BEGIN
			...
			BEGIN
				...
				END
			END
			ELSE IF(@EntryTypeName LIKE 'ChiPhiMuaHang%')
			BEGIN
				IF (@ReceiptCostTotalFees > 0)
				Begin
				insert into @ReceiptItemsToPostTable
				 select (case when ii.FK_ACAccountID = 0
							then @DefaultDebitAccount
					...
					, (case when ii.FK_ACFeePaymentAccountID = 0
							then @DefaultCreditAcount
						else ii.FK_ACFeePaymentAccountID
						end)
					, SUM(ii.ReceiptItemCostFees) / @ReceiptExchangeRate
					, sum(ii.ReceiptItemCostFees)
					, 0
					, 0
					, @ReceiptExchangeRate
					, @EntryTypeName
					...
					, ii.ICReceiptItemCode10Combo			
				 from @ReceiptItemTable ii
				 WHERE ii.ReceiptItemCostFees > 0
				 group by  
					 ii.FK_ACAccountID
					, ii.FK_APPurchaseOrderID
					, ii.FK_APInvoiceInID
				...
				insert into @ReceiptItemsToPostTable
				 select  (case when ii.FK_ACFeePaymentAccountID = 0
							then @DefaultCreditAcount
						else ii.FK_ACFeePaymentAccountID
						end)
					, (case when ii.FK_ACAccountID = 0
