...
namespace BOSERP.Utilities
{
    ...
    public static class Code128Rendering
    {
        ...
        public static Image MakeQRImage(string InputData)
        {
            using (QRCoder.QRCodeGenerator qrGenerator = new QRCoder.QRCodeGenerator())
            using (QRCodeData qrCodeData = qrGenerator.CreateQrCode(InputData, QRCoder.QRCodeGenerator.ECCLevel.Q))
            using (QRCode qrCode = new QRCode(qrCodeData))
            {
                Bitmap qrCodeImage = qrCode.GetGraphic(20);
                Image image = (Image)qrCodeImage;
                return image;
            }
        }
        ...
        /// <param name="AddQuietZone">Add required horiz margins (use if output is tight)</param>
        /// <returns>An Image of the Code128 barcode representing the message</returns>
        public static Image MakeBarcodeLargeImage(string InputData, int BarWeight, bool AddQuietZone)
        {
            // get the Code128 codes to represent the message
            Code128Content content = new Code128Content(InputData);
            int[] codes = content.Codes;
