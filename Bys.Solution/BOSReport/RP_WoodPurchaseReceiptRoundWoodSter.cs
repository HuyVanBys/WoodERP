...
namespace BOSReport
{
    public partial class RP_WoodPurchaseReceiptRoundWoodSter : BaseReport
    {
        public RP_WoodPurchaseReceiptRoundWoodSter()
        {
            InitializeComponent();
        ...
        {
            double text = double.Parse(xrl_KhoiLuongSter.Text.ToString());
            double so = Math.Round(text, 4);
            xrl_KhoiLuongSter.Text = string.Format("{0:n4}", so);
        }
