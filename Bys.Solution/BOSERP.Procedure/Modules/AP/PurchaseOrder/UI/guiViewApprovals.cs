...
{
    public partial class guiViewApprovals : BOSERPScreen
    {
        #region Variables
        private GridControlHelper GridControlHelper;

        public List<APPurchaseOrderApprovalsInfo> ProposalApprovals { get; set; }
        #endregion

        public guiViewApprovals(List<APPurchaseOrderApprovalsInfo> proposalApprovals)
        {
            InitializeComponent();
        ...
        private void cardView1_CustomDrawCardCaption(object sender, DevExpress.XtraGrid.Views.Card.CardCaptionCustomDrawEventArgs e)
        {
            ...
            APPurchaseOrderApprovalsInfo item = (APPurchaseOrderApprovalsInfo)view.GetRow(e.RowHandle);
            if (item == null)
                return;
            
            Brush backBrush, foreBrush;
            switch (item.APPurchaseOrderApprovalAction)
            {
                case "Approve":
            ...
            string caption = BOSApp.GetDisplayTextFromConfigValue("PurchaseOrderApprovalAction", item.APPurchaseOrderApprovalAction);
