...
namespace BOSERP.Modules.Product
{
    public partial class ProductionNormItemsHardwareTreeListControl : BOSTreeListControl
    {
        ...
        public void AddTreeListColumns(int visibleIndex, string fieldName, bool readOnly, bool AllowEdit)
        {
            ...
            {
                column.Caption = ProductLocalizedResources.MMProductionNormItemICProductNameHardware;
                column.Fixed = DevExpress.XtraTreeList.Columns.FixedStyle.Left;
                column.Width = 250;
                column.BestFit();
            }
            ...
            {
                column.Caption = ProductLocalizedResources.MMProductionNormItemICProductNoHardware;
                column.Width = 350;
                column.Fixed = DevExpress.XtraTreeList.Columns.FixedStyle.Left;
                //column.BestFit();
            }
            ...
            if (dsColumns.Tables.Count > 0)
            {
                foreach (DataRow rowColumn in dsColumns.Tables[0].Rows)
                {
                    ...
                    if (objAAColumnAliasInfo != null)
                    {
                        ...
                        {
                            column.Caption = ProductLocalizedResources.MMProductionNormItemDescCaption;
                            column.OptionsColumn.AllowEdit = true;
                            column.OptionsColumn.ReadOnly = false;
                        }
                        if (fieldName == "MMProductionNormItemHardwareAllocationQuantity")
                            column.Caption = ProductLocalizedResources.MMProductionNormItemHardwareSubForSets;
                        //if (fieldName == ProductionNormConst.MMProductionNormItemQuantity)
                        ...
                        if (fieldName == "MMProductionNormItemProductSizeAndPacking")
                            column.Caption = ProductLocalizedResources.MMHardwareItemSpecification;
                        if (fieldName == "MMProductionNormItemProductDesc")
                            column.Caption = ProductLocalizedResources.MMProductionNormItemSemiProductDepartmentAttributeCaption;
                        if (fieldName == ProductionNormConst.MMProductionNormItemSortOrder)
                        {
                            column.Caption = ProductionNormConst.MMProductionNormOrderNoCaption;
                        }
                        ...
                        {
                            column.Caption = ProductLocalizedResources.MMProductionNormItemHardwareAssembleTotalQuantityCaption;
                        }
                    }
        ...
        private void RpItemComponent_Click(object sender, EventArgs e)
        {
            ...
            MMProductionNormItemsInfo objProductionNormItemsInfo = (MMProductionNormItemsInfo)entity.ProductionNormItemHardwareList.CurrentObject;
            ((ProductModule)Screen.Module).ShowProductionNormItemHardwareDetail(objProductionNormItemsInfo);
        }
        ...
        //            ((ProductModule)Screen.Module).InvalidateProductionNormInventoryStockGridControl(objProductionNormItemsInfo.FK_ICProductID, ProductType.Hardware.ToString());
        //    }
        //}
        //public void repositoryItemUnitLookUpEdit_QueryPopUp(object sender, System.ComponentModel.CancelEventArgs e)
        //{
        //    LookUpEdit lookUpEdit = (LookUpEdit)sender;
        //    DataTable dt = ((ProductModule)Screen.Module).GetMeasureUnit();
