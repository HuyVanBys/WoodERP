...
namespace BOSERP.Modules.SemiProductReceipt
{
    ...
    public class SemiProductReceiptModule : GLReceiptModule
    {
        ...
        {
            int workShopID = 0;
            if (fld_lkeFK_MMWorkShopID != null && fld_lkeFK_MMWorkShopID.EditValue != null) int.TryParse(fld_lkeFK_MMWorkShopID.EditValue.ToString(), out workShopID);
            DataSet ds = new DataSet();
            if (BOSApp.LookupTables.ContainsKey("MMLines"))
                ds = (DataSet)BOSApp.LookupTables["MMLines"];
            if (fld_lkeFK_LineID != null && workShopID > 0)
            {
                if (!BOSApp.LookupTables.ContainsKey("MMLines"))
                {
