...
{
    public partial class RP124 : ReportForm
    {
        /// <summary>
        ...
        private void fld_btnExportExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    fld_dgcRP124ACLoanReceipts.ExportToXlsx(fileName);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            int objectID = 0;
            if (fld_lkeCSCompanyBankID.EditValue != null)
                int.TryParse(fld_lkeCSCompanyBankID?.EditValue.ToString(), out objectID);
            DateTime searchFromDate = fld_dteSearchFromDate.DateTime;
            DateTime searchToDate = fld_dteSearchToDate.DateTime;
            ADReportsController objReportsController = new ADReportsController();
            List<ACLoanReceiptsInfo> loanReceiptList = objReportsController.GetACLoanReceiptForReport(objectID, searchFromDate, searchToDate);
            fld_dgcRP124ACLoanReceipts.DataSource = loanReceiptList;
