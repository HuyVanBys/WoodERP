...
{
    public partial class RP059 : ReportForm
    {
        /// <summary>
        ...
        #region Events
        private void RP059_Load(object sender, EventArgs e)
        {
        ...
        private void fld_btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (currencyID > 0)
            {
                if (currencyID == GECurrencyID.VND)
                    report = new BOSReport.RP059VND();
                else
                    report = new BOSReport.RP059();
            }
            else
            {
                report = new BOSReport.RP059Currency();
            }
        ...
        private void fld_chkSelectAll_CheckedChanged(object sender, EventArgs e)
        {
            ...
            fld_dgcACRP059Accounts.RefreshDataSource();
        }
        #endregion
        ...
        public void InitAccounts()
        {
            ...
            fld_dgcACRP059Accounts.DataSource = SelectedAccountList;
        }
        ...
        public bool InitReportDataSource(XtraReport report)
        {
            ...
            {
                currencyName = objCurrenciesController.GetObjectNameByID(100000);
            }
        ...
        /// <summary>
        /// Get documents relating to selected accounts
        /// </summary>
        /// <param name="fromDate">Date the result is filtered from</param>
        /// <param name="toDate">Date the result is filtered to</param>
        /// <returns>List of documents</returns>
        private List<ACDocumentsInfo> GetDocumentsBySelectedAccounts(DateTime fromDate, DateTime toDate)
        {
            ...
            int branchID = Convert.ToInt32(fld_lkeBRBranchID.EditValue);
            //BRBranchsController objBranchsController = new BRBranchsController();
            //List<BRBranchsInfo> selectedBranches = GetSelectedBranchesByLocationIDAndBranchID(null, branchID);
            //BRBranchsInfo objBranchsInfo = (BRBranchsInfo)objBranchsController.GetObjectByID(branchID);
            List<ACDocumentsInfo> documents;
            decimal startBalance = 0;
            foreach (ACAccountsInfo objAccountsInfo in SelectedAccountList)
            {
                if (objAccountsInfo.Selected)
                {
                    documents = new List<ACDocumentsInfo>();
                    startBalance = 0;
                    //foreach (BRBranchsInfo branch in selectedBranches)
                    //{
                    List<ACDocumentsInfo> list = objDocumentsController.GetDocumentsByAccountID(objAccountsInfo.ACAccountID, currencyID, null, branchID, fromDate, toDate);
                    documents.AddRange(list);

                    if (currencyID > 0)
                    {
                        startBalance += objDocumentEntrysController.GetAccountBalanceByCurrency(objAccountsInfo.ACAccountNo, fromDate.AddDays(-1), currencyID, 0, branchID);
                    }
                    else
                    {
                        startBalance += objDocumentEntrysController.GetAccountBalance(objAccountsInfo.ACAccountNo, fromDate.AddDays(-1), currencyID, 0, branchID);
                    }
                    //}
                    documents = documents.OrderBy(item => item.ACDocumentDate).ToList();
                    documents = documents.Where(d => d.ACDocumentTypeName == AccDocumentType.PhieuThu.ToString() ||
                                                d.ACDocumentTypeName == AccDocumentType.PhieuChi.ToString() ||
                                                d.ACDocumentTypeName == AccDocumentType.ThanhToanTrucTiep.ToString()).ToList();
                    documents.ForEach(d => d.ACAccountNo = string.Format("{0} - {1}", objAccountsInfo.ACAccountNo, objAccountsInfo.ACAccountName));
                    documents.ForEach(d => d.GECurrencyName = currencyName);

                    decimal endBalance = startBalance;
                    foreach (ACDocumentsInfo document in documents)
                    {
                        if (document.ACDocumentTypeName == AccDocumentType.PhieuThu.ToString())
                        {
                            document.ReceiptDocumentNo = document.ACDocumentNo;
                            if (currencyID > 0)
                            {
                                document.DocumentReceiptAmount = document.ACDocumentTotalAmount;
                            }
                            else
                            {
                                document.DocumentReceiptAmount = document.ACDocumentExchangeAmount;
                            }
                        }
                        else if (document.ACDocumentTypeName == AccDocumentType.PhieuChi.ToString() ||
                            document.ACDocumentTypeName == AccDocumentType.ThanhToanTrucTiep.ToString())
                        {
                            document.PaymentDocumentNo = document.ACDocumentNo;
                            if (currencyID > 0)
                            {
                                document.DocumentPaymentAmount = document.ACDocumentTotalAmount;
                            }
                            else
                            {
                                document.DocumentPaymentAmount = document.ACDocumentExchangeAmount;
                            }
                        }
                        endBalance = endBalance + document.DocumentReceiptAmount - document.DocumentPaymentAmount;
                        document.AccountBalance = endBalance;
                    }

                    if (documents.Count == 0 && startBalance != 0)
                    {
                        ACDocumentsInfo document = new ACDocumentsInfo();
                        document.ACAccountNo = string.Format("{0} - {1}", objAccountsInfo.ACAccountNo, objAccountsInfo.ACAccountName);
                    ...
                    {
                        documents[0].StartPeriodBalance = startBalance;
                        documents[documents.Count - 1].EndPeriodBalance = endBalance;
                    }
                    result = result.Concat(documents).ToList();
                }
            }
            return result;
        }
