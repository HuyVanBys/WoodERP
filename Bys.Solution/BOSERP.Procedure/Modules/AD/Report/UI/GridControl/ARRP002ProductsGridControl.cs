...
{
    public partial class APRP002ProductsGridControl : ReportGridControl
    {
        /// <summary>
        ...
        /// </summary>
        public List<ICProductGroupsInfo> ProductGroupList { get; set; }
        ...
        public void AddDataSource(DataTable table, DateTime dateFrom, DateTime dateTo, int dateType)
        {
            this.DataSource = table;
            BandedGridView bandedView = InitializeBandedGridView(GridViewMain, dateFrom, dateTo, dateType);
            MainView = bandedView;
        ...
        private BandedGridView InitializeBandedGridView(GridView gridView, DateTime dateFrom, DateTime dateTo, int dateType)
        {
            ...
            BandedGridColumn gridColumn = CreateNewColumn("ICProductGroupParentName", ReportLocalizedResources.ICProductGroupParentName, gridBand);
            BandedGridColumn gridColumnDepartment = CreateNewColumn("ICDepartmentName", ReportLocalizedResources.ICDepartmentName, gridBand);
            gridColumn.Visible = false;
            gridColumnDepartment.Visible = false;
            CreateNewColumn("ICProductGroupName", ReportLocalizedResources.ICProductGroupName, gridBand);
            ...
            while (index <= dateTo)
            {
                if (dateType == 0)
                {
                    ...
                    BandedGridColumn column = CreateNumericColumn(quantityColumnName, ReportLocalizedResources.ARInvoiceItemProductQty, band);
                    ...
                    BandedGridColumn totalColumn = CreateNumericColumn(totalColumnName, ReportLocalizedResources.ARInvoiceItemTotalAmount, band);
                    ...
                    BandedGridColumn percentCol = CreateNewColumn(percentColName, ReportLocalizedResources.RateIncreaseDecrease, band);
                    percentCol.BestFit();
                ...
                {
                    string bandName = index.ToString("yyyy");
                    GridBand band = CreateNewGridBand(bandName, bandName);
                    bands.Add(band);
                    bandedView.Bands.Add(band);
