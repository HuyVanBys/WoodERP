...
{
    public partial class RP107MMAllocationProposalsGridControl : ReportGridControl
    {
        ...
        void gridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (gridView.FocusedColumn != null)
            {
                ...
                {
                    MMAllocationProposalsInfo item = (MMAllocationProposalsInfo)gridView.GetRow(gridView.FocusedRowHandle);
                    if (item != null)
                    {
                        (((RP107)Screen)).Getalldata(item.MMAllocationProposalID, item.MMAllocationProposalProductType);
                    }
                }
            }
        }
