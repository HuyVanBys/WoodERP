...
{
    public partial class RP045 : ReportForm
    {
        public RP045()
        {
            InitializeComponent();
        ...
        {
            BOSReport.RP045 report = new BOSReport.RP045();
            //report.LoadLayout(BOSCommon.Constants.Report.DevRP045Path);
            InitDataSourceReport(report);
            guiReportPreview reviewer = new guiReportPreview(report, BOSCommon.Constants.Report.DevRP045Path, true);
            reviewer.Show();
        }
        ...
        public void InitDataSourceReport(XtraReport report)
        {
            ...
            List<ACDocumentEntrysInfo> documentEntryList = documentEntrys.Where(d => d.ACDocumentTypeName == AccDocumentType.MuaHangCongNoNhapKhau.ToString()).ToList();
            if (documentEntryList.Count > 0)
                report.DataSource = documentEntryList;

        }

        private void RP045_Load(object sender, EventArgs e)
        {
