...
{
    public partial class RP048 : ReportForm
    {
        public RP048()
        {
            InitializeComponent();
        ...
        {
            BOSReport.RP048 report = new BOSReport.RP048();
        ...
        public void InitDataSourceReport(XtraReport report)
        {
            ...
            List<ACDocumentEntrysInfo> documentEntrys = objReportsController.GetDocumentEntryList(dateFrom, dateTo, 0);
            List<ACDocumentEntrysInfo> documentEntryList = documentEntrys.Where(d => d.ACDocumentTypeName == AccDocumentType.BanHangCongNo.ToString()).ToList();
            if (documentEntryList.Count > 0)
                report.DataSource = documentEntryList;

        }

        private void RP048_Load(object sender, EventArgs e)
        {
