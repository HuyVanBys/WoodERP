...
{
    public partial class RP052 : ReportForm
    {
        ...
        {
            BOSReport.RP052 report = new BOSReport.RP052();
        ...
        public void InitDataSourceReport(XtraReport report)
        {
            ...
            if (fld_chkSearchByStocks.Checked == true)
            {
                inventoryStocks = objReportsController.GetIOInventoryListBySomeCriteriaByStockNoListSearchByStock(dateFrom, dateTo, stockNoList,
                                                                                                                                productID, accountID, supplierID);
            }
            else
            {
                inventoryStocks = objReportsController.GetIOInventoryListBySomeCriteriaByStockNoList(dateFrom, dateTo, stockNoList,
                                                                                                                                productID, accountID, supplierID);
            }
            //NUThao [EDIT] [10/16/2014] [DB centre] [Report Permission], END
            ...
            List<ICInventoryStocksInfo> prevInventoryStocks = new List<ICInventoryStocksInfo>();
            if (fld_chkSearchByStocks.Checked == true)
            {
                prevInventoryStocks = objReportsController.GetIOInventoryListBySomeCriteriaByStockNoListSearchByStock(null, dateFrom.AddDays(-1), stockNoList,
                                                                                                                                    productID, accountID, supplierID);
            }
            else
            {
                prevInventoryStocks = objReportsController.GetIOInventoryListBySomeCriteriaByStockNoList(null, dateFrom.AddDays(-1), stockNoList,
                                                                                                                                    productID, accountID, supplierID);
            }
            //NUThao [EDIT] [10/16/2014] [DB centre] [Report Permission], END
            ...
            {
                foreach (ICInventoryStocksInfo item in inventoryStockList)
                {
                    if (!string.IsNullOrEmpty(StockName) && item.ICStockName != null
                                && !(StockName.Contains(item.ICStockName)))
                    {
                        StockName += ", ";
                    }
                    if (item.ICStockName != null && !(StockName.Contains(item.ICStockName)))
                    {
                        StockName += item.ICStockName;
                    }
                }
            }
